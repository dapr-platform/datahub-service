...
var BasicLibraryViews = map[string]string{
	...
	"sync_tasks_info": `
		...
		SELECT 
			...
			-- 基础库信息对象，来源：basic_libraries表（通过data_source关联）
			-- 包含字段：id, name_zh, name_en, description, status, created_at, created_by, updated_at, updated_by
			jsonb_build_object(
				'id', bl.id,
				'name_zh', bl.name_zh,
		...
		INNER JOIN basic_libraries bl ON ds.library_id = bl.id
		LEFT JOIN data_interfaces di ON st.interface_id = di.id;
		
		COMMENT ON VIEW sync_tasks_info IS '{
			"description": "同步任务详细信息视图：聚合同步任务基本信息及其关联的数据源、接口和基础库信息",
			"fields": {
