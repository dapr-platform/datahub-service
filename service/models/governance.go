...
type QualityRuleTemplate struct {
	...
	RuleLogic     JSONB     `gorm:"type:jsonb;not null" json:"rule_logic"`     // 规则逻辑模板
...
type QualityRuleConfig struct {
	...
	RuntimeConfig  JSONB    `json:"runtime_config"` // 运行时配置
	Threshold      JSONB    `json:"threshold"`      // 阈值配置
...
type DataMaskingTemplate struct {
	...
	Description     string         `gorm:"type:text" json:"description"`
	ApplicableTypes pq.StringArray `gorm:"type:text[]" json:"applicable_types"`             // 适用的数据类型
	MaskingLogic    JSONB          `gorm:"type:jsonb;not null" json:"masking_logic"`        // 脱敏逻辑模板
	Parameters      JSONB          `gorm:"type:jsonb" json:"parameters"`                    // 可配置参数定义
	DefaultConfig   JSONB          `gorm:"type:jsonb" json:"default_config"`                // 默认配置
...
type DataMaskingConfig struct {
	...
	TargetFields     []string `json:"target_fields"`     // 目标字段列表
	MaskingConfig    JSONB    `json:"masking_config"`    // 运行时脱敏配置
	ApplyCondition   string   `json:"apply_condition"`   // 应用条件
	PreserveFormat   bool     `json:"preserve_format"`   // 是否保持格式
	ReversibleConfig JSONB    `json:"reversible_config"` // 可逆配置（如果支持）
	IsEnabled        bool     `json:"is_enabled"`
}
...
type SystemLog struct {
	...
	OperationContent JSONB     `gorm:"type:jsonb;not null" json:"operation_content"`
...
type DataQualityReport struct {
	...
	QualityMetrics    JSONB     `gorm:"type:jsonb;not null" json:"quality_metrics"`
