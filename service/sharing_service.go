...
// CreateDataSyncTask 创建数据同步任务
func (s *SharingService) CreateDataSyncTask(task *models.DataSyncTask) error {
	// 验证源类型
	validSourceTypes := []string{"database", "api", "file"}
	isValidSourceType := false
	for _, validType := range validSourceTypes {
		if task.SourceType == validType {
	...
	// 验证目标类型
	validTargetTypes := []string{"database", "api", "file"}
	isValidTargetType := false
	for _, validType := range validTargetTypes {
		if task.TargetType == validType {
	...
	// 验证同步策略
	validStrategies := []string{"full", "incremental"}
	isValidStrategy := false
	for _, validStrategy := range validStrategies {
		if task.SyncStrategy == validStrategy {
			isValidStrategy = true
			break
		}
	}
	if !isValidStrategy {
		return errors.New("无效的同步策略")
	}

	return s.db.Create(task).Error
}

// GetDataSyncTasks 获取数据同步任务列表
func (s *SharingService) GetDataSyncTasks(page, pageSize int, status, createdBy string) ([]models.DataSyncTask, int64, error) {
	var tasks []models.DataSyncTask
	var total int64

	query := s.db.Model(&models.DataSyncTask{}).Preload("Creator")
...
// GetDataSyncTaskByID 根据ID获取数据同步任务
func (s *SharingService) GetDataSyncTaskByID(id string) (*models.DataSyncTask, error) {
	var task models.DataSyncTask
	if err := s.db.Preload("Creator").First(&task, "id = ?", id).Error; err != nil {
		return nil, err
	}
	return &task, nil
}

// UpdateDataSyncTask 更新数据同步任务
func (s *SharingService) UpdateDataSyncTask(id string, updates map[string]interface{}) error {
	return s.db.Model(&models.DataSyncTask{}).Where("id = ?", id).Updates(updates).Error
}

// DeleteDataSyncTask 删除数据同步任务
func (s *SharingService) DeleteDataSyncTask(id string) error {
	return s.db.Delete(&models.DataSyncTask{}, "id = ?", id).Error
}

// === 数据同步日志管理 ===

// CreateDataSyncLog 创建数据同步日志
func (s *SharingService) CreateDataSyncLog(log *models.DataSyncLog) error {
	return s.db.Create(log).Error
}

// GetDataSyncLogs 获取数据同步日志列表
func (s *SharingService) GetDataSyncLogs(page, pageSize int, taskID, status string) ([]models.DataSyncLog, int64, error) {
	var logs []models.DataSyncLog
	var total int64

	query := s.db.Model(&models.DataSyncLog{}).Preload("Task")
